...
@DisplayName("DocumentVersionDao Integration Tests")
class DocumentVersionRepositoryTest {
    ...
    void setUp() {
        ...
        sample.setRef("documentversion-test-ref-001");
        sample.setVersionNumber(1);
        sample.setOriginalFilename("test-originalfilename");
        sample.setStoredFilename("test-storedfilename");
        sample.setStorageKey("test-storagekey");
        sample.setFileSizeBytes(1L);
        sample.setMimeType("test-mimetype");
    }
    ...
    class Save {
        ...
        void persistsEntity() {
            DocumentVersion saved = dao.save(sample);

            assertThat(saved.getId()).isNotNull().isPositive();
            assertThat(saved.getRef()).isEqualTo("documentversion-test-ref-001");
        }
    ...
    class FindById {
        ...
        void returnsSavedEntity() {
            ...
            Optional<DocumentVersion> found = dao.findById(saved.getId());
        ...
        void returnsEmpty_whenNotFound() {
            Optional<DocumentVersion> found = dao.findById(-999L);
    ...
    class FindByRef {
        ...
        void returnsEntityByRef() {
            ...
            DocumentVersion found = dao.findByRef("documentversion-test-ref-001");

            assertThat(found).isNotNull();
            assertThat(found.getRef()).isEqualTo("documentversion-test-ref-001");
        }

        @Test
        @DisplayName("returns null for unknown ref")
        void returnsNull_whenRefNotFound() {
            DocumentVersion found = dao.findByRef("non-existent-ref");

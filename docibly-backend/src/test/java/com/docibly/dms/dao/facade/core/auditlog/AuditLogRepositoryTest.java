package com.docibly.dms.dao.facade.core.auditlog;
...
@DisplayName("AuditLogDao Integration Tests")
class AuditLogRepositoryTest {
    ...
    void setUp() {
        sample = new AuditLog();
        sample.setRef("auditlog-test-ref-001");
    }
    ...
    class Save {
        ...
        void persistsEntity() {
            AuditLog saved = dao.save(sample);

            assertThat(saved.getId()).isNotNull().isPositive();
            assertThat(saved.getRef()).isEqualTo("auditlog-test-ref-001");
        }
    ...
    class FindById {
        ...
        void returnsSavedEntity() {
            ...
            Optional<AuditLog> found = dao.findById(saved.getId());
        ...
        void returnsEmpty_whenNotFound() {
            Optional<AuditLog> found = dao.findById(-999L);
    ...
    class FindByRef {
        ...
        void returnsEntityByRef() {
            ...
            AuditLog found = dao.findByRef("auditlog-test-ref-001");

            assertThat(found).isNotNull();
            assertThat(found.getRef()).isEqualTo("auditlog-test-ref-001");
        }

        @Test
        @DisplayName("returns null for unknown ref")
        void returnsNull_whenRefNotFound() {
            AuditLog found = dao.findByRef("non-existent-ref");

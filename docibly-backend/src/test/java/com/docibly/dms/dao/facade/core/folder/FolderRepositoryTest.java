...
@DisplayName("FolderDao Integration Tests")
class FolderRepositoryTest {
    ...
    void setUp() {
        sample = new Folder();
        sample.setRef("folder-test-ref-001");
    ...
    class Save {
        ...
        void persistsEntity() {
            Folder saved = dao.save(sample);

            assertThat(saved.getId()).isNotNull().isPositive();
            assertThat(saved.getRef()).isEqualTo("folder-test-ref-001");
        }
    ...
    class FindById {
        ...
        void returnsSavedEntity() {
            ...
            Optional<Folder> found = dao.findById(saved.getId());
        ...
        void returnsEmpty_whenNotFound() {
            Optional<Folder> found = dao.findById(-999L);
    ...
    class FindByRef {
        ...
        void returnsEntityByRef() {
            ...
            Folder found = dao.findByRef("folder-test-ref-001");

            assertThat(found).isNotNull();
            assertThat(found.getRef()).isEqualTo("folder-test-ref-001");
        }

        @Test
        @DisplayName("returns null for unknown ref")
        void returnsNull_whenRefNotFound() {
            Folder found = dao.findByRef("non-existent-ref");

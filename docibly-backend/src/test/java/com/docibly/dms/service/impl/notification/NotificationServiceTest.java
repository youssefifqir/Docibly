...
class NotificationServiceTest {
    ...
    void setUp() {
        ...
        sample.setId(1L);
        sample.setRef("notification-ref-001");
    }
    ...
    class FindById {
        ...
        void returnsNull_whenNotFound() {
            ...
            Optional<Notification> result = service.findById(99L);
    ...
    class Save {
        ...
        void persistsEntity() {
            ...
            when(dao.save(any(Notification.class))).thenReturn(sample);

            Notification result = service.save(toSave);

            assertThat(result).isNotNull();
            assertThat(result.getId()).isEqualTo(1L);
            verify(dao).save(any(Notification.class));
        }
        ...
        void generatesRef_whenMissing() {
            Notification toSave = new Notification();
            when(dao.save(any(Notification.class))).thenAnswer(inv -> inv.getArgument(0));
        ...
        void returnsNull_whenEntityIsNull() {
            Notification result = service.save(null);
    ...
    class Create {
        ...
        void delegatesToSave() {
            ...
            Notification result = service.create(toCreate);
    ...
    class FindByRef {
        ...
        void returnsEntity_whenRefMatches() {
            when(dao.findByRef("notification-ref-001")).thenReturn(sample);

            Notification result = service.findByRef("notification-ref-001");

            assertThat(result).isNotNull();
            assertThat(result.getRef()).isEqualTo("notification-ref-001");
            verify(dao).findByRef("notification-ref-001");
        }

        @Test
        @DisplayName("returns null for blank ref without hitting the DAO")
        void returnsNull_whenRefIsBlank() {
            Notification result = service.findByRef("  ");

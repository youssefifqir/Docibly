...
class DocumentServiceTest {
    ...
    void setUp() {
        sample = new Document();
        sample.setId(1L);
        sample.setRef("document-ref-001");
    }
    ...
    class FindById {
        ...
        void returnsEntity_whenFound() {
            ...
            Optional<Document> result = service.findById(1L);
        ...
        void returnsNull_whenNotFound() {
            ...
            Optional<Document> result = service.findById(99L);
    ...
    class Save {
        ...
        void persistsEntity() {
            ...
            when(dao.save(any(Document.class))).thenReturn(sample);

            Document result = service.save(toSave);
        ...
        void generatesRef_whenMissing() {
            Document toSave = new Document();
            when(dao.save(any(Document.class))).thenAnswer(inv -> inv.getArgument(0));
        ...
        void returnsNull_whenEntityIsNull() {
            Document result = service.save(null);
    ...
    class Create {
        ...
        void delegatesToSave() {
            ...
            Document result = service.create(toCreate);
    ...
    class FindByRef {
        ...
        void returnsEntity_whenRefMatches() {
            when(dao.findByRef("document-ref-001")).thenReturn(sample);

            Document result = service.findByRef("document-ref-001");

            assertThat(result).isNotNull();
            assertThat(result.getRef()).isEqualTo("document-ref-001");
            verify(dao).findByRef("document-ref-001");
        }

        @Test
        @DisplayName("returns null for blank ref without hitting the DAO")
        void returnsNull_whenRefIsBlank() {
            Document result = service.findByRef("  ");

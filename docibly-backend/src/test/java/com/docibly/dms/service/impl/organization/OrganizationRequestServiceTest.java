...
@DisplayName("OrganizationRequestService Unit Tests")
class OrganizationRequestServiceTest {

    @Mock
    private OrganizationRequestDao dao;

    @InjectMocks
    private OrganizationRequestServiceImpl service;

    private OrganizationRequest sample;

    @BeforeEach
    void setUp() {
        sample = new OrganizationRequest();
        sample.setId(1L);
        sample.setRef("organizationrequest-ref-001");
    }
    ...
    class FindAll {
        ...
        void returnsEntities() {
            ...
            List<OrganizationRequest> result = service.findAll();
    ...
    class FindById {
        ...
        void returnsEntity_whenFound() {
            ...
            Optional<OrganizationRequest> result = service.findById(1L);
        ...
        void returnsNull_whenNotFound() {
            ...
            Optional<OrganizationRequest> result = service.findById(99L);
    ...
    class Save {
        ...
        void persistsEntity() {
            OrganizationRequest toSave = new OrganizationRequest();
            when(dao.save(any(OrganizationRequest.class))).thenReturn(sample);

            OrganizationRequest result = service.save(toSave);

            assertThat(result).isNotNull();
            assertThat(result.getId()).isEqualTo(1L);
            verify(dao).save(any(OrganizationRequest.class));
        }
        ...
        void generatesRef_whenMissing() {
            OrganizationRequest toSave = new OrganizationRequest();
            when(dao.save(any(OrganizationRequest.class))).thenAnswer(inv -> inv.getArgument(0));
        ...
        void returnsNull_whenEntityIsNull() {
            OrganizationRequest result = service.save(null);
    ...
    class Create {
        ...
        void delegatesToSave() {
            OrganizationRequest toCreate = new OrganizationRequest();
            when(dao.save(any(OrganizationRequest.class))).thenReturn(sample);

            OrganizationRequest result = service.create(toCreate);
    ...
    class FindByRef {
        ...
        void returnsEntity_whenRefMatches() {
            when(dao.findByRef("organizationrequest-ref-001")).thenReturn(sample);

            OrganizationRequest result = service.findByRef("organizationrequest-ref-001");

            assertThat(result).isNotNull();
            assertThat(result.getRef()).isEqualTo("organizationrequest-ref-001");
            verify(dao).findByRef("organizationrequest-ref-001");
        }

        @Test
        @DisplayName("returns null for blank ref without hitting the DAO")
        void returnsNull_whenRefIsBlank() {
            OrganizationRequest result = service.findByRef("  ");

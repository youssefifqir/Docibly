...
@DisplayName("OrganizationMemberService Unit Tests")
class OrganizationMemberServiceTest {
    ...
    private OrganizationMember sample;

    @BeforeEach
    void setUp() {
        sample = new OrganizationMember();
        sample.setId(1L);
        sample.setRef("organizationmember-ref-001");
    }
    ...
    class FindAll {
        ...
        void returnsEntities() {
            ...
            List<OrganizationMember> result = service.findAll();
    ...
    class FindById {
        ...
        void returnsEntity_whenFound() {
            ...
            Optional<OrganizationMember> result = service.findById(1L);
        ...
        void returnsNull_whenNotFound() {
            ...
            Optional<OrganizationMember> result = service.findById(99L);
    ...
    class Save {
        ...
        void persistsEntity() {
            OrganizationMember toSave = new OrganizationMember();
            when(dao.save(any(OrganizationMember.class))).thenReturn(sample);

            OrganizationMember result = service.save(toSave);

            assertThat(result).isNotNull();
            assertThat(result.getId()).isEqualTo(1L);
            verify(dao).save(any(OrganizationMember.class));
        }
        ...
        void generatesRef_whenMissing() {
            OrganizationMember toSave = new OrganizationMember();
            when(dao.save(any(OrganizationMember.class))).thenAnswer(inv -> inv.getArgument(0));
        ...
        void returnsNull_whenEntityIsNull() {
            OrganizationMember result = service.save(null);
    ...
    class Create {
        ...
        void delegatesToSave() {
            OrganizationMember toCreate = new OrganizationMember();
            when(dao.save(any(OrganizationMember.class))).thenReturn(sample);

            OrganizationMember result = service.create(toCreate);
    ...
    class FindByRef {
        ...
        void returnsEntity_whenRefMatches() {
            when(dao.findByRef("organizationmember-ref-001")).thenReturn(sample);

            OrganizationMember result = service.findByRef("organizationmember-ref-001");

            assertThat(result).isNotNull();
            assertThat(result.getRef()).isEqualTo("organizationmember-ref-001");
            verify(dao).findByRef("organizationmember-ref-001");
        }

        @Test
        @DisplayName("returns null for blank ref without hitting the DAO")
        void returnsNull_whenRefIsBlank() {
            OrganizationMember result = service.findByRef("  ");

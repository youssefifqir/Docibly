...
@DisplayName("DocumentCommentService Unit Tests")
class DocumentCommentServiceTest {

    @Mock
    private DocumentCommentDao dao;

    @InjectMocks
    private DocumentCommentServiceImpl service;

    private DocumentComment sample;

    @BeforeEach
    void setUp() {
        sample = new DocumentComment();
        sample.setId(1L);
        sample.setRef("documentcomment-ref-001");
    }
    ...
    class FindAll {
        ...
        void returnsEntities() {
            ...
            List<DocumentComment> result = service.findAll();
    ...
    class FindById {
        ...
        void returnsEntity_whenFound() {
            ...
            Optional<DocumentComment> result = service.findById(1L);
        ...
        void returnsNull_whenNotFound() {
            ...
            Optional<DocumentComment> result = service.findById(99L);
    ...
    class Save {
        ...
        void persistsEntity() {
            DocumentComment toSave = new DocumentComment();
            when(dao.save(any(DocumentComment.class))).thenReturn(sample);

            DocumentComment result = service.save(toSave);

            assertThat(result).isNotNull();
            assertThat(result.getId()).isEqualTo(1L);
            verify(dao).save(any(DocumentComment.class));
        }
        ...
        void generatesRef_whenMissing() {
            DocumentComment toSave = new DocumentComment();
            when(dao.save(any(DocumentComment.class))).thenAnswer(inv -> inv.getArgument(0));
        ...
        void returnsNull_whenEntityIsNull() {
            DocumentComment result = service.save(null);
    ...
    class Create {
        ...
        void delegatesToSave() {
            DocumentComment toCreate = new DocumentComment();
            when(dao.save(any(DocumentComment.class))).thenReturn(sample);

            DocumentComment result = service.create(toCreate);
    ...
    class FindByRef {
        ...
        void returnsEntity_whenRefMatches() {
            when(dao.findByRef("documentcomment-ref-001")).thenReturn(sample);

            DocumentComment result = service.findByRef("documentcomment-ref-001");

            assertThat(result).isNotNull();
            assertThat(result.getRef()).isEqualTo("documentcomment-ref-001");
            verify(dao).findByRef("documentcomment-ref-001");
        }

        @Test
        @DisplayName("returns null for blank ref without hitting the DAO")
        void returnsNull_whenRefIsBlank() {
            DocumentComment result = service.findByRef("  ");

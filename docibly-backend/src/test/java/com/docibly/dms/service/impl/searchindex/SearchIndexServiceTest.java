...
class SearchIndexServiceTest {
    ...
    private SearchIndex sample;

    @BeforeEach
    void setUp() {
        sample = new SearchIndex();
        sample.setId(1L);
        sample.setRef("searchindex-ref-001");
    }
    ...
    class FindAll {
        ...
        void returnsEntities() {
            ...
            List<SearchIndex> result = service.findAll();
    ...
    class FindById {
        ...
        void returnsEntity_whenFound() {
            ...
            Optional<SearchIndex> result = service.findById(1L);
        ...
        void returnsNull_whenNotFound() {
            ...
            Optional<SearchIndex> result = service.findById(99L);
    ...
    class Save {
        ...
        void persistsEntity() {
            SearchIndex toSave = new SearchIndex();
            when(dao.save(any(SearchIndex.class))).thenReturn(sample);

            SearchIndex result = service.save(toSave);

            assertThat(result).isNotNull();
            assertThat(result.getId()).isEqualTo(1L);
            verify(dao).save(any(SearchIndex.class));
        }
        ...
        void generatesRef_whenMissing() {
            SearchIndex toSave = new SearchIndex();
            when(dao.save(any(SearchIndex.class))).thenAnswer(inv -> inv.getArgument(0));
        ...
        void returnsNull_whenEntityIsNull() {
            SearchIndex result = service.save(null);
    ...
    class Create {
        ...
        void delegatesToSave() {
            SearchIndex toCreate = new SearchIndex();
            when(dao.save(any(SearchIndex.class))).thenReturn(sample);

            SearchIndex result = service.create(toCreate);
    ...
    class FindByRef {
        ...
        void returnsEntity_whenRefMatches() {
            when(dao.findByRef("searchindex-ref-001")).thenReturn(sample);

            SearchIndex result = service.findByRef("searchindex-ref-001");

            assertThat(result).isNotNull();
            assertThat(result.getRef()).isEqualTo("searchindex-ref-001");
            verify(dao).findByRef("searchindex-ref-001");
        }

        @Test
        @DisplayName("returns null for blank ref without hitting the DAO")
        void returnsNull_whenRefIsBlank() {
            SearchIndex result = service.findByRef("  ");

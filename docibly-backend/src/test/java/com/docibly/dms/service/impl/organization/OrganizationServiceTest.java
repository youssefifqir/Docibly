...
class OrganizationServiceTest {
    ...
    private Organization sample;

    @BeforeEach
    void setUp() {
        sample = new Organization();
        sample.setId(1L);
        sample.setRef("organization-ref-001");
    }
    ...
    class FindById {
        ...
        void returnsNull_whenNotFound() {
            ...
            Optional<Organization> result = service.findById(99L);
    ...
    class Save {
        ...
        void persistsEntity() {
            ...
            when(dao.save(any(Organization.class))).thenReturn(sample);

            Organization result = service.save(toSave);

            assertThat(result).isNotNull();
            assertThat(result.getId()).isEqualTo(1L);
            verify(dao).save(any(Organization.class));
        }
        ...
        void generatesRef_whenMissing() {
            Organization toSave = new Organization();
            when(dao.save(any(Organization.class))).thenAnswer(inv -> inv.getArgument(0));
        ...
        void returnsNull_whenEntityIsNull() {
            Organization result = service.save(null);
    ...
    class Create {
        ...
        void delegatesToSave() {
            ...
            Organization result = service.create(toCreate);
    ...
    class FindByRef {
        ...
        void returnsEntity_whenRefMatches() {
            when(dao.findByRef("organization-ref-001")).thenReturn(sample);

            Organization result = service.findByRef("organization-ref-001");

            assertThat(result).isNotNull();
            assertThat(result.getRef()).isEqualTo("organization-ref-001");
            verify(dao).findByRef("organization-ref-001");
        }

        @Test
        @DisplayName("returns null for blank ref without hitting the DAO")
        void returnsNull_whenRefIsBlank() {
            Organization result = service.findByRef("  ");

...
class DocumentVersionServiceTest {
    ...
    @Mock
    private DocumentVersionDao dao;

    @InjectMocks
    private DocumentVersionServiceImpl service;

    private DocumentVersion sample;

    @BeforeEach
    void setUp() {
        sample = new DocumentVersion();
        sample.setId(1L);
        sample.setRef("documentversion-ref-001");
    }
    ...
    class FindAll {
        ...
        void returnsEntities() {
            ...
            List<DocumentVersion> result = service.findAll();
    ...
    class FindById {
        ...
        void returnsEntity_whenFound() {
            ...
            Optional<DocumentVersion> result = service.findById(1L);
        ...
        void returnsNull_whenNotFound() {
            ...
            Optional<DocumentVersion> result = service.findById(99L);
    ...
    class Save {
        ...
        void persistsEntity() {
            DocumentVersion toSave = new DocumentVersion();
            when(dao.save(any(DocumentVersion.class))).thenReturn(sample);

            DocumentVersion result = service.save(toSave);

            assertThat(result).isNotNull();
            assertThat(result.getId()).isEqualTo(1L);
            verify(dao).save(any(DocumentVersion.class));
        }
        ...
        void generatesRef_whenMissing() {
            DocumentVersion toSave = new DocumentVersion();
            when(dao.save(any(DocumentVersion.class))).thenAnswer(inv -> inv.getArgument(0));
        ...
        void returnsNull_whenEntityIsNull() {
            DocumentVersion result = service.save(null);
    ...
    class Create {
        ...
        void delegatesToSave() {
            DocumentVersion toCreate = new DocumentVersion();
            when(dao.save(any(DocumentVersion.class))).thenReturn(sample);

            DocumentVersion result = service.create(toCreate);
    ...
    class FindByRef {
        ...
        void returnsEntity_whenRefMatches() {
            when(dao.findByRef("documentversion-ref-001")).thenReturn(sample);

            DocumentVersion result = service.findByRef("documentversion-ref-001");

            assertThat(result).isNotNull();
            assertThat(result.getRef()).isEqualTo("documentversion-ref-001");
            verify(dao).findByRef("documentversion-ref-001");
        }

        @Test
        @DisplayName("returns null for blank ref without hitting the DAO")
        void returnsNull_whenRefIsBlank() {
            DocumentVersion result = service.findByRef("  ");

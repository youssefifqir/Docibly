...
class AuditLogServiceTest {
    ...
    void setUp() {
        sample = new AuditLog();
        sample.setId(1L);
        sample.setRef("auditlog-ref-001");
    }
    ...
    class FindById {
        ...
        void returnsEntity_whenFound() {
            ...
            Optional<AuditLog> result = service.findById(1L);
        ...
        void returnsNull_whenNotFound() {
            ...
            Optional<AuditLog> result = service.findById(99L);
    ...
    class Save {
        ...
        void persistsEntity() {
            AuditLog toSave = new AuditLog();
            when(dao.save(any(AuditLog.class))).thenReturn(sample);

            AuditLog result = service.save(toSave);

            assertThat(result).isNotNull();
            assertThat(result.getId()).isEqualTo(1L);
            verify(dao).save(any(AuditLog.class));
        }
        ...
        void generatesRef_whenMissing() {
            AuditLog toSave = new AuditLog();
            when(dao.save(any(AuditLog.class))).thenAnswer(inv -> inv.getArgument(0));
        ...
        void returnsNull_whenEntityIsNull() {
            AuditLog result = service.save(null);
    ...
    class Create {
        ...
        void delegatesToSave() {
            AuditLog toCreate = new AuditLog();
            when(dao.save(any(AuditLog.class))).thenReturn(sample);

            AuditLog result = service.create(toCreate);
    ...
    class FindByRef {
        ...
        void returnsEntity_whenRefMatches() {
            when(dao.findByRef("auditlog-ref-001")).thenReturn(sample);

            AuditLog result = service.findByRef("auditlog-ref-001");

            assertThat(result).isNotNull();
            assertThat(result.getRef()).isEqualTo("auditlog-ref-001");
            verify(dao).findByRef("auditlog-ref-001");
        }

        @Test
        @DisplayName("returns null for blank ref without hitting the DAO")
        void returnsNull_whenRefIsBlank() {
            AuditLog result = service.findByRef("  ");
